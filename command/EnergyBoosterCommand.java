...
public class EnergyBoosterCommand
implements CommandExecutor,
TabCompleter {
    private final BoosterService boosterService;

    public EnergyBoosterCommand(BoosterService boosterService) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 4 || !args[0].equalsIgnoreCase("give")) {
            sender.sendMessage(Text.color("&cUsage: /energybooster give <player> <duration> <multiplier>"));
            return true;
        }
        ...
        ItemStack item = this.boosterService.createBoosterItem(BoosterService.BoosterType.ENERGY, durationSeconds, multiplier, 1);
        target.getInventory().addItem(new ItemStack[]{item});
        sender.sendMessage(Text.color("&aGave Charge booster to &f" + target.getName()));
        return true;
    }
